...
class Pipeline:
    ...
    def _Flare_Filtering(self, rate, median, std):
        three_sigma = 3 * std
    ...
    def Dataset_Download(self, ObsID_wanted, ObsID_excluded, ObsID_Chosen):
        ...
        table = table[table["name"] == self.star_name]
    ...
    def Time_Plot(self):
        ...
        for i, obsid in enumerate(self.ObsID_array):
            ...
            with fits.open(fits_file) as plot:
                ...
                plt.scatter(time_array[flares_filter], data["RATE"][flares_filter], color="red")
                plt.scatter(time_array[below_threshold], data["RATE"][below_threshold], color="gray")
                plt.scatter(time_array[normal], data["RATE"][normal], color="blue")
                ...
                plt.title(f"{self.star_name} {obsid} barycentered")
                plt.xlim(0, self.orbital_period)
